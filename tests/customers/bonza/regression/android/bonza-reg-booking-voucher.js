...
const finalPrice = [];
const bookingNumber = [];
let finalTotalPriceAmountBeforeVoucher = [];
let voucher = data.coupons.PROD.VOUCHER1;
...
describe('Home', ()=>{
    ...
    it('Select seats',async ()=> {
        ...
        depardSeatPrice.push((await departSeatPriceElement.getText()).replace(/[^0-9.]+/g, ''))
        ...
        returnSeatPrice.push((await returnSeatPriceElement.getText()).replace(/[^0-9.]+/g, ''))
    ...
    it('Validate all pricess before apply the voucher',async ()=>{
    ...
    it('Apply invalid and valid voucher',async ()=>{
        // Get total price before apply the voucher
        let totalPriceElement = driver.$(bonzaReviewBookingPage.main.totalPrice)
        let totalPriceAmountBeforeVoucher = (await totalPriceElement.getText()).replace(/[^0-9.]+/g, '');
        finalTotalPriceAmountBeforeVoucher.push(totalPriceAmountBeforeVoucher)

        // Click on apply voucher checkbox
        await global.clickOn(bonzaReviewBookingPage.main.voucherCheckbox)
        // Add invalid voucher value
        await global.addValue(bonzaReviewBookingPage.main.voucherInput, 'FAKEVOUCHER')
        // Click on apply button
        await global.clickOn(bonzaReviewBookingPage.main.voucherApplyButton)
        // Validate invalid voucher message
        await global.validateElementText(bonzaReviewBookingPage.main.voucherPopupMessageText, 'Invalid voucher, please verify voucher information')
        // Click on ok button in the invalid voucher popup
        await global.clickOn(bonzaReviewBookingPage.main.voucherOKButton)

        // Clear input field
        await global.clearValue(bonzaReviewBookingPage.main.voucherInput)
        // Add valid voucher value
        await global.addValue(bonzaReviewBookingPage.main.voucherInput, voucher)
        // Click on apply button
        await global.clickOn(bonzaReviewBookingPage.main.voucherApplyButton)
        // Validate valid voucher message
        await global.validateElementText(bonzaReviewBookingPage.main.voucherPopupMessageText, 'You have succesfully applied your voucher, please check your updated summary')
        // Click on ok button in the valid voucher popup
        await global.clickOn(bonzaReviewBookingPage.main.voucherOKButton)
    });

    it('Validate all pricess after apply the voucher',async ()=>{
        // Get the voucher amount from the summary list
        let voucherElement = driver.$('//android.view.ViewGroup[@content-desc="touchable-opacity__general__collapse__toggle-confirmation-and-payment-price-summary"]/android.widget.TextView[10]')
        let voucherAmount = (await voucherElement.getText()).replace(/[^0-9.]+/g, '');
        console.log(`finalTotalPriceAmountBeforeVoucher: $${finalTotalPriceAmountBeforeVoucher}`)
        console.log(`voucherAmount is: $${voucherAmount}`)
        // Calculate the total price minus the voucher
        let totalPriceIncludingVoucher = finalTotalPriceAmountBeforeVoucher - voucherAmount;
        console.log(`totalPriceIncludingVoucher is: $${totalPriceIncludingVoucher.toFixed(2)}`)
        ...
        // Validate bags price
        await global.validateElementText(bonzaReviewBookingPage.main.bagsPriceValue, allBagsPrice)
        // Validate voucher price
        await global.validateElementText(bonzaReviewBookingPage.main.voucherAppliedValue, voucherAmount)
        // Validate total price
        await global.validateElementText(bonzaReviewBookingPage.main.totalPrice, totalPriceIncludingVoucher.toFixed(2))
